...
const Patient = () => {
    ...
    const handlePrevious = () => {
        if (patients.length) setCurrentPatientIndex((prev) => (prev > 0 ? prev - 1 : patients.length - 1));
    };
    const handleNext = () => {
        if (patients.length) setCurrentPatientIndex((prev) => (prev < patients.length - 1 ? prev + 1 : 0));
    };
    ...
    const handleSOS = () => {
        Alert.alert('Hätäapu', `Soitetaanko ${coordinatorName}, ${coordinatorPhoneDisplay}?`, [
            { text: 'Peruuta', style: 'cancel' },
            {
                ...
                onPress: () => {
                    ...
                    Linking.canOpenURL(tel).then((supported) => {
                        ...
                        else Alert.alert('Virhe', 'Puhelinsoittoa ei voida aloittaa tällä laitteella.');
                    });
                },
                style: 'destructive',
            },
            ...
            <ScrollView style={styles.container}>
                ...
                <View style={styles.card}>
                    <View style={styles.noteHeader}>
                        ...
                        <TouchableOpacity style={[styles.micButton, isRecording ? styles.micActive : styles.micInactive]} onPress={handleVoiceInput}>
                            <Ionicons name="mic" size={20} color="#fff" />
                        </TouchableOpacity>
                    </View>

                    <TextInput style={styles.textarea} placeholder="Kirjoita muistiinpanot tänne..." multiline value={noteText} onChangeText={setNoteText} />

                    <TouchableOpacity style={[styles.saveButton, !noteText.trim() && styles.saveButtonDisabled]} onPress={handleSaveNote} disabled={!noteText.trim()}>
...
const styles = StyleSheet.create({
    ...
    sosContainer: { flexDirection: 'row', justifyContent: 'center', marginBottom: 16 },
    sosCombined: { flexDirection: 'row', alignItems: 'center', backgroundColor: '#f3f4f6', borderRadius: 15, paddingVertical: 6, paddingHorizontal: 8 },
    sosIconButton: { width: 36, height: 36, borderRadius: 18, backgroundColor: '#dc2626', alignItems: 'center', justifyContent: 'center' },
    sosLabelCombined: { marginLeft: 12, fontSize: 16, fontWeight: '600', color: '#111827' },
